      ...
      if (token && date) {
        try {
          ...
          const { data } = await axios.get(
            `http://localhost:5000/api/v1/cita/filtrar/${formattedDate}`,
            {
              headers: {
      ...
      <div className="flex flex-col items-center px-10 h-screen">
        <div className="w-full flex bg-primaryBlue">
          ...
          <div className="w-2/3 p-4 grid grid-cols-3 gap-4">
            <CountCard
              title="Realizadas"
              count={citasStats.realizadas}
              color="bg-green-500"
              onClick={() => setFilter("realizadas")}
              isActive={filter === "realizadas"}
            />
            <CountCard
              title="En vivo"
              count={citasStats.enVivo}
              color="bg-yellow-500"
              onClick={() => setFilter("enVivo")}
              isActive={filter === "enVivo"}
            />
            <CountCard
              title="Próximas"
              count={citasStats.proxima}
              color="bg-red-500"
              onClick={() => setFilter("proxima")}
              isActive={filter === "proxima"}
            />
          </div>
        </div>
        <div className="w-full">
          <h1
            className={`my-10 ${getHeaderBgColor()} text-secondaryBlue p-2 text-4xl font-bold text-center`}
          >
            {getHeaderTitle()}
          </h1>
...
const CountCard = ({ title, count, color, onClick, isActive }) => (
  <div
    className={`p-4 rounded-lg shadow-md ${color} text-white cursor-pointer ${
      isActive ? "opacity-70" : ""
    }`}
    onClick={onClick}
  >
      ...
      <div className="w-full flex flex-row">
        ...
        <div className="w-8/12 mt-5 flex">
          <div className="w-1/2">
            <div className="flex items-center">
              ...
              />
              <h1 className="ml-2 text-secondaryBlue font-bold text-xl">
                {servicio}
              </h1>
            </div>
        ...
        <div className="w-2/12 mt-5 justify-center pr-10">
          <div className="bg-white flex flex-col items-center justify-center space-y-4">
            <button className="px-6 py-2 font-medium bg-primaryBlue text-secondaryBlue w-full transition-all shadow-[3px_3px_0px_black] hover:shadow-none hover:translate-x-[3px] hover:translate-y-[3px]">
              Ver cita
            </button>
          </div>
        </div>
