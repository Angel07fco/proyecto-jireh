...
function AdminGaleria() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`http://localhost:5000/api/v1/galeria`)
    ...
    if (
      ...
      try {
        const response = await axios.post(
          "http://localhost:5000/api/v1/galeria",
          data
        );
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:5000/api/v1/galeria/${id}`
      );
      setResponseSuccess(response.data.msj);
    } catch (error) {
      if (error.response) {
  ...
  return (
    <AdminLayout>
      <HeaderAdmin texto="GALERIA ADMIN" linkText="galeria administrador" />
      {loading && <Loader />}
      ...
      <div className="px-10">
        ...
        <Button
          texto="Agregar una nueva galeria"
