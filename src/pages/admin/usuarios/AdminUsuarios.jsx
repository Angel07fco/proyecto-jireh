...
function AdminUsuarios() {
  ...
  useEffect(() => {
    ...
    if (token) {
      axios
        .get(`http://localhost:5000/api/v1/user/admingetusers/usuario`, {
          headers: {
  ...
  const confirmHabilitar = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/v1/user/habilitar/${id}`,
        {},
        {
          headers: {
            "x-access-token": token,
  ...
  const confirmDeshabilitar = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/v1/user/deshabilitar/${id}`,
        {},
        {
          headers: {
            "x-access-token": token,
      ...
      <div className="px-10">
        ...
        <Modal open={open5} onClose={() => setOpen5(false)}>
          <div className="mx-auto my-4 w-96 mb-3">
            ...
            <p className="text-sm text-gray-500 text-center mt-3">
              ¿Estas seguro que deseas bloquear esta cuenta?
            </p>
          </div>
          <div className="flex gap-10 mt-10">
            <button
