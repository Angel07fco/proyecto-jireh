...
function AdminFaq() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`http://localhost:5000/api/v1/preguntas-frecuentes`)
  ...
  const handleRegisterFaq = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/v1/preguntas-frecuentes/",
        data
      );
  ...
  const confirmUpdate = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/v1/preguntas-frecuentes/${id}`,
        data
      );
      setResponseSuccess(response.data.msj);
      console.log(response.data.msj);
  ...
  return (
    <AdminLayout>
      ...
      <div className="px-10">
        ...
        <Button
          texto="Agregar una nueva pregunta"
          onClick={() => setOpen(true)}
        />
        <CrudTableFaq data={db} editData={editData} deleteData={deleteData} />
        ...
        <Modal open={open2} onClose={() => setOpen2(false)}>
          <div className="mx-auto my-4 w-96 mb-3">
            ...
            <p className="text-sm text-gray-500 text-center mt-3">
              ¿Estas seguro que deseas eliminar la Pregunta Frecuente?
            </p>
          </div>
