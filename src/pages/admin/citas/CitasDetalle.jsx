  ...
  const handleSubmitDelete = async () => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:5000/api/v1/cita/cancelar/${cita._id}`,
        {
          headers: {
  ...
  const hanSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/v1/cita/actualizar/${cita._id}`,
        data,
        {
          headers: {
    ...
    if (startDate !== "") {
      ...
      try {
        const response = await axios.get(
          `http://localhost:5000/api/v1/cita/citas/${medico}/${formattedDate}`,
          {
            headers: {
              "x-access-token": token,
            },
        ...
        <div className="w-full h-[100vh] overflow-y-scroll">
          <div className="mx-10">
            ...
            <div
              ...
              className="bg-red-500 text-white mt-5 p-3 text-xl text-center rounded-xl hover:bg-white hover:text-red-500 border hover:border-red-500 cursor-pointer"
            >
              Cancelar cita
            </div>
                ...
                <div className="w-full">
                  <div className="flex justify-between mb-1">
                    <div className="w-full">
                      ...
                      <Input disabled value={cita.mascota} />
                    </div>
                  </div>
                  ...
                  {comentario ? (
                    ...
                    <Input disabled value={cita.comentarios} />
                  )}
                  ...
                  {renderServicio ? (
                    ...
                    <Input disabled value={cita.servicio} />
                  )}
                  ...
                  <div className="mt-8 mb-8">
                    ...
                    {renderDetalles ? (
                      <>
                        {medico === "" ? (
                          <>
                            <h1 className="font-medium text-secondaryBlue text-lg my-3">
                            ...
                            <div className="w-full grid grid-cols-2 gap-5 mb-5">
                              {medicos.map((item, index) => (
                                <div
                                  ...
                                  className={`border flex border-secondaryBlue cursor-pointer rounded-xl p-5 ${
                                    selectedMedicoIndex === index
                                      ? "bg-primaryBlue"
                                      : ""
                                  }`}
                                >
                                  ...
                                  <div className="ml-3">
                                    ...
                                    <h1 className="text-secondaryBlue">
                                      {item.nombre}
                                    </h1>
                                  </div>
                                </div>
                          ...
                          <h1 className="border border-secondaryBlue p-1 text-center text-xl">
                            Medico ya seleccionado
                          </h1>
                        )}
                        ...
                        {nuevosHorarios.length === 0 ? (
                          ...
                          <h1 className="border border-secondaryBlue p-1 text-center text-xl mt-5">
                            Fecha ya seleccionado
                          </h1>
                        )}
                          ...
                          {nuevosHorarios.length === 0 ? (
                            <div className="w-full border border-red-500 flex items-center justify-center p-2">
                              <h1 className="text-red-500 text-xs font-bold">
                                Para agendar la hora de la cita primero debe
                                comprobar la disponibilidad de horarios del
                                Médico y el dia.
                              </h1>
                            </div>
                          ) : (
                      ...
                      <div className="space-y-3">
                        <div>
                          <Label>Médico</Label>
                          <Input disabled value={cita.medico} />
                        </div>
                        <div>
                          <Label>Fecha</Label>
                          <Input disabled value={cita.fecha} />
                        </div>
                        <div>
                          <Label>Hora</Label>
                          <Input disabled value={cita.hora} />
                        </div>
                      </div>
                    )}
                  </div>
