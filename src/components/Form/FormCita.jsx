        ...
        <div>
            <ol className="flex mt-10 items-center w-full text-sm font-medium text-center sm:text-base">
                ...
                <li className={`flex md:w-full items-center after:content-[''] after:w-full after:h-1 after:border-b after:border-gray-200 after:border-1 after:hidden sm:after:inline-block after:mx-6 xl:after:mx-10 dark:after:border-gray-700`}>
                    <div className={`w-8 h-8 flex items-center justify-center border-2 border-secondaryBlue  rounded-full ${step !== 1 ? 'bg-secondaryBlue' : ''}`}>
                        <h1 className={` ${step !== 1 ? 'text-primaryBlue' : 'text-secondaryBlue'}`}>2</h1>
                    </div>
                </li>
                <li className={`flex items-center `}>
                    <div className={`w-8 h-8 flex items-center justify-center border-2 border-secondaryBlue  rounded-full ${step === 3 ? 'bg-secondaryBlue' : ''}`}>
                        <h1 className={` ${step === 3 ? 'text-primaryBlue' : 'text-secondaryBlue'}`}>3</h1>
                    </div>
                </li>
            </ol>

            {step === 1 && <CardForm1 onNext={handleNext} onSelectVet={handleSelectVet} />}
            {step === 2 && <CardForm2 onNext={handleNext} veterinario={selectedVetId} onBack={handleBack} onSelectFecha={handleSelectFecha} onSelectHora={handleSelectHora} />}
            {step === 3 && <CardForm3 onBack={handleBack} onRegister={handleRegister} veterinario={selectedVetId} fecha={selectedFecha} hora={selectedHora} />}
        ...
        <div className="w-full bg-white h-auto justify-center items-center rounded-xl">
            ...
            <div className='w-full flex space-x-10 py-10'>
                {medicos.map((item, index) => (
                    <div
                        key={index}
                        className={`border border-secondaryBlue cursor-pointer rounded-xl p-5 ${selectedMedicoIndex === index ? 'bg-primaryBlue' : ''}`}
...
function CardForm2({ onNext, onBack, veterinario, onSelectFecha, onSelectHora }) {
    ...
    return (
        <div className="w-full h-auto flex flex-col rounded-xl cursor-pointer">
            <div className='flex items-center bg-secondaryBlue p-2 mt-10'>
                ...
                <h1 className='ml-2 text-primaryBlue text-xl'>Seleccione dia y hora de la cita</h1>
            </div>
            ...
            <div className='flex space-x-10'>
                <Button onClick={onBack} texto="Atrás">Atrás</Button>
                {canProceed === true
                    ?
                        <Button onClick={onNext} texto="Siguiente" />
                    :
    ...
    const handleConfirmCita = async () => {
        ...
        try {
            const response = await axios.post(
                "https://backend-jireh.onrender.com/api/v1/cita/newcita",
                data,
                {
                    headers: {
                        "x-access-token": token
    ...
    return (
        <div className="w-full h-auto flex flex-col rounded-xl cursor-pointer">
            ...
            <div className='flex items-center bg-secondaryBlue p-2 mt-10'>
                ...
                <h1 className='ml-2 text-primaryBlue text-xl'>Mascota, servicio y comentarios</h1>
            </div>

            <div className='my-5'>
                <h1 className='font-medium text-lg mb-3  text-secondaryBlue'>Seleccione un servicio:</h1>
                <div className='grid grid-cols-5 gap-2'>
                    {services.map((item, index) => (
                        <div
                            key={index}
                            onClick={() => handleCardClick(index)}
                            className={`border border-secondaryBlue p-2 rounded-xl flex flex-col justify-center items-center ${selectedServiceIndex === index ? 'bg-primaryBlue' : ''}`}
                        >
            ...
            <div className='mb-5'>
                <div className="mt-2">
                    <div className='flex justify-between items-center'>
                        <h1 className='font-medium text-lg mb-3  text-secondaryBlue'>Seleccione una mascota:</h1>
                    ...
                    <div className='grid grid-cols-3 gap-2'>
                        {pets.map((item, index) => (
                            <div
                                ...
                                className={`border border-secondaryBlue p-2 rounded-xl flex flex-col justify-center items-center ${selectedMascotaIndex === index ? 'bg-primaryBlue' : ''}`}
                            >
            ...
            <Modal open={open} onClose={() => setOpen(false)}>
                <div className='mx-auto my-4 w-96 mb-3'>
                    <h1 className='text-lg font-black text-gray-800 text-center'>Confirmar cita</h1>
                    <p className='text-sm text-gray-500 text-center mt-3'>¿Deseas agendar tu cita?</p>
                </div>
