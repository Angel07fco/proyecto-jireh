...
function Perfil() {
    ...
    const handleObtenerToken = async () => {
        ...
        const response = await axios.post(
            `https://backend-jireh.onrender.com/api/v1/tokenacceso/${token}/${tipoToken}`
        );
                ...
                <div className="w-full h-[100vh] overflow-y-scroll">
                    ...
                    <div className="my-10 mx-20">
                        <div className="w-full flex">
                            ...
                            <div className="w-2/5 flex flex-col space-y-5 items-center justify-center">
                                <HoverTiltCard
                ...
                <div className='mx-auto my-4 w-96 mb-3'>
                    {
                        ...
                        :
                            <>
                                <h1 className='text-lg font-black text-secondaryBlue text-center'>{ModalTexto}</h1>
                                <p className='text-sm text-secondaryBlue font-bold text-center mt-3'>El token solo es de un uso.</p>
                            </>
                    }
                </div>
                            ...
                            <div className='w-full flex flex-col justify-center items-center'>
                                <div className='w-full flex justify-center items-center space-x-2'>
                                    {tokenAcceso.split('').map((char, index) => (
                                        <p key={index} className='bg-secondaryBlue text-primaryBlue text-lg font-bold pt-1 pb-2 px-3 flex justify-center items-center rounded-md'>
                                            {char}
                                        </p>
                                    ))}
                                </div>
                                <p className='text-sm text-red-600 font-bold text-center mt-3'>Nota: Este token solo funcionara en tu dispostivo {dispositivo}</p>
                                <button
                                    onClick={cerrarModal}
...
function Modal({ open, onClose, children, imageUrl }) {
    return (
        <div className={`fixed z-50 inset-0 flex justify-center items-center transition-colors ${open ? 'visible bg-gray-900 bg-opacity-70' : 'invisible'}`}>
            <div className="absolute inset-0 bg-gray-900 opacity-40" />
            <div
                className={`relative bg-white rounded-xl shadow p-6 transition-all ${open ? 'scale-100 opacity-100' : 'scale-125 opacity-0'}`}
            >
                <div
