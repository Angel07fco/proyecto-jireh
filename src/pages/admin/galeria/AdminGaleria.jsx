...
import CrudTableGaleria from "./CrudTableGaleria";
import axios from "axios";
import Label from "../../../components/Ui/Label";
import { arrayGaleria } from "../../../helpers/ArrayGaleria";
...
function AdminGaleria() {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(
            `https://backend-jireh.onrender.com/api/v1/galeria`
        )
    ...
    const handleRegisterGaleria = async () => {
        if ( titulo === "" || descripcion === "" || selectedClasificacion === "" || selectedCategoria === "" || selectedImage === "" ) {
    ...
    const clasificacionSeleccionada = arrayGaleria.find(
        (item) => item.clasificacionesbd === selectedClasificacion
    );
    ...
    const confirmDelete = async () => {
        setLoading(true);
        try {
            const response = await axios.delete(`https://backend-jireh.onrender.com/api/v1/galeria/${id}`);
            setResponseSuccess(response.data.msj);
        } catch (error) {
            if (error.response) {
            setResponseErrors(error.response.data);
    ...
    const confirmUpdate = async () => {
        if ( titulo === "" || descripcion === "" || selectedClasificacion === "" || selectedCategoria === "" || selectedImage === "" ) {
            ...
            try {
                const response = await axios.put(
                    `https://backend-jireh.onrender.com/api/v1/galeria/${id}`,
                    data
                );
                setResponseSuccess(response.data.msj);
                console.log(response.data.msj);
    ...
    return (
        <AdminLayout>
            <HeaderAdmin
                texto="GALERIA ADMIN"
                linkText="galeria administrador"
            />
            {loading && <Loader />}
            ...
            <div className="px-10">
                ...
                <Button texto="Agregar una nueva galeria" onClick={() => setOpen(true)} />
                <CrudTableGaleria
                    data={db}
                    editData={editData}
                ...
                <Modal open={open} onClose={() => setOpen(false)}>
                    ...
                    <div className="my-5">
                        ...
                        >
                            ...
                            {arrayGaleria.map((item, index) => (
                                <option key={index} value={item.clasificacionesbd}>
                    ...
                    <div className="my-5 w-full">
                        <Label>Imagen</Label>
                        <div className="w-[80%] ml-[10%] rounded-full mb-5">
                            {selectedImage && (
                ...
                <Modal open={open2} onClose={() => setOpen2(false)}>
                    <div className='mx-auto my-4 w-96 mb-3'>
                        <h1 className='text-lg font-black text-gray-800 text-center'>Eliminar Galeria</h1>
                        <p className='text-sm text-gray-500 text-center mt-3'>¿Estas seguro que deseas eliminar esta galeria?</p>
                    </div>
                    <div className="flex gap-10 mt-10">
                        <button
