...
function AdminUsuarios() {
    ...
    const confirmHabilitar = async () => {
        ...
        try {
            const response = await axios.put(
                `https://backend-jireh.onrender.com/api/v1/user/habilitar/${id}`,
                {},
                {
                    headers: {
                        "x-access-token": token
    ...
    const confirmDeshabilitar = async () => {
        ...
        try {
            const response = await axios.put(
                `https://backend-jireh.onrender.com/api/v1/user/deshabilitar/${id}`,
                {},
                {
                    headers: {
                        "x-access-token": token
            ...
            <div className="px-10">
                ...
                <Modal open={open4} onClose={() => setOpen4(false)}>
                    <div className='mx-auto my-4 w-96 mb-3'>
                        <h1 className='text-lg font-black text-gray-800 text-center'>Activar Cuenta</h1>
                        <p className='text-sm text-gray-500 text-center mt-3'>¿Estas seguro que deseas activar esta cuenta?</p>
                    </div>
                ...
                <Modal open={open5} onClose={() => setOpen5(false)}>
                    <div className='mx-auto my-4 w-96 mb-3'>
                        <h1 className='text-lg font-black text-gray-800 text-center'>Bloquear Cuenta</h1>
                        <p className='text-sm text-gray-500 text-center mt-3'>¿Estas seguro que deseas bloquear esta cuenta?</p>
                    </div>
                    <div className="flex gap-10 mt-10">
                        <button
                            onClick={handleCanceled}
